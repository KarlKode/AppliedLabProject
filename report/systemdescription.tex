...
\end{itemize}
The likelihood that a potential vulnerability could exploited by a threat-source can be described as high, medium, or low. The table below describes the three likelihood levels. The definition if the levels as well as the definition of likelihood are based on and to some parts adopted from the NIST Guide for Conducting Risk Assessments \footnote{NIST Special Publication 800-30 2002: Risk Management Guide for Information Technology Systems}.
...
\newcounter{threadnr}\addtocounter{threadnr}{1}
...
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Nature: Environmental hazards like flood, lightning, etc. & Server room is on the first floor, lightning protection for the whole building & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Nature: Power outage & UPS for the critical systems, redundant power supplies on different electric circuits &  {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Nature: Component failure & Service contract with manufacturer, spare machines, backup & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Insiders: Accidental demolition by employees & Service contract with manufacturer, spare machines, backup & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Insiders \& Industrial espionage: Unauthorized physical access & The server room is locked, only system administrators have access & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}

The asset Internal network and Internet connectivity have the same threads and countermeasures as described for the other physical assets, but they have some additional threads which are evaluated here:
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Insiders: System administrator misconfigured the network & Labels clearly show which cable has to go where, only migrate to production if checked by colleague  & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Nature: ISP is down, land line is cut & Service level agreement with the ISP, consider redundancy  & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Malware: Unaware users with out of date software & User training, make up-to-date software mandatory, install/update anti virus/malware programs, firewall shields internal network from internet & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Insiders: Employees misconfigure software such that system is unusable & Limit access rights of users, trainings, backup & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Script kiddie: gains control over workstation, uses it to issue certificates & workstations not directly accessible from the internet, use antivirus software, maintain workstations &{\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Skilled hacker \& Industrial espionage: gain control over workstation, use it to compromise CA & workstations not directly accessible from the internet, use antivirus software, maintain workstations, intrusion detection, monitoring traffic & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Insiders: Normal users use their proximity to try to break the firewall, administrators misconfigure firewall & Intrusion detection via logging, log reviews, reviews before deployment, backup & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Skilled hacker: Overtakes firewall & Intrusion detection through logs, limit accessibility from the outside, exponential timeout after failed login attempts, system administrator regularly updates the system & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Industrial espionage: Overtakes firewall, in addition to skilled hacker may purchase zero day attack & The same countermeasures as in threat \addtocounter{threadnr}{-1}\arabic{threadnr}\addtocounter{threadnr}{2}, empathize updating the system  & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Script kiddies: Denial of service attack against firewall & Contract with the ISP to prevent/act on denial of service attacks & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Skilled hacker \& Industrial espionage: Get access to the system & Stop all unnecessary services, only allow administrators to login, reject all unneeded network traffic, keep software up to date, log all activities, review logs, exponential timeout after failed login attempts & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Script kiddies: Get access to the system & Not easily accessible from the internet with other than HTTP(S), keep software up to date, log all activities, review logs, exponential timeout after failed login attempts & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Script kiddies \& malicious Insiders: Get access to the system & Not accessible from the internet or office intranet, keep software up to date, log all activities, review logs, exponential timeout after failed login attempts & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Script kiddies \& Skilled hacker: XSS/SQL injection & Sanitize all input, do not write own queries & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Script kiddies: Denial of service & Nginx countermeasures activated & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Skilled hackers: Run/inject own code & Minimize all information leakage about system components, version, status etc., run application with low privileged user & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Skilled hacker: Hijack session, eavesdrop on communication & HTTPS only & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Skilled hackers: Run/inject own code & Review logs, make updates, minimize access, run application with low privileged user, not accessible from outside the server intranet & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
\arabic{threadnr}\addtocounter{threadnr}{1} & Script kiddies \& Skilled hacker: XSS/SQL injection & Sanitize all input, do not write own queries & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Skilled hackers: Run/inject own code & Review logs, make updates, minimize access, limit operations user can execute, not accessible from outside the server intranet & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Insiders \& Skilled hacker: Read/alter logs & Permissions set such that no one can alter/delete logs, encrypted transmission, only administrators can read logs & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Skilled hacker: Insert false logging information & Only accept data from specific sources, Authenticated and encrypted SSH tunnel, static ARP tables against poisoning & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Insiders \& Skilled hacker: Change configuration, use SSH & Allowed users specified, only administrators have access to configurations, keep software up to date & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Insiders \& Skilled hacker: Read/alter backup & Permissions set such that no one can alter/delete backups, encrypted transmission, only administrators can read backups & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Skilled hacker: Insert own data & Only accept data from specific sources, Authenticated and encrypted SSH tunnel, static ARP tables against poisoning & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Script kiddies \& Skilled hacker: Obtain user data & Sanitize all input, do not write own queries, for password encrypt transmission, HTTPS from user to webserver, SSH tunnel from webserver to CA Core server  & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Insiders: Input wrong user data & Review by CA administrator & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Script kiddies \& Skilled hacker \& Insiders: Alter user data (not own) & Do not write own queries, do not allow delete or insert for database user & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Skilled hacker \& Insiders: Obtain credentials & Enforce strong passwords, exponential timeouts after failed login attempts against bruteforce attack & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} &  &  & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Insiders: Disclose private key due to insecure storage of private key on workstation & Train users how to handle keys, harden systems & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Skilled hacker: Obtain private key & Transmission from CA Core via webserver to client is encrypted and authenticated, key on CA Core deleted after sending, encrypted for archive & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Insiders: Change information linked to certificate & Approval form CA administrator for changes, automatically revoke certificates & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Skilled hacker: Issue/use certificate & Do not allow user creation, approval form CA administrator for changes on user data & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Insider \& Skilled hacker: Alter/read logs & Transmitted via SSH tunnel, only readable by administrator & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Insider \& Skilled hacker: Alter/read configs  & Transmitted via SSH tunnel, only readable by administrator, backed up & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Insider \& Skilled hacker: Obtain key archive & Only administrators have access, transmitted via SSH tunnel, only specific connections allowed & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Skilled hacker: Disclose private key(s) & Encryption of private key, encryption key stored off site & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Skilled hacker: Eavesdrop & Transmitted via SSH tunnel & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Skilled hacker: Inject/alter & Transmitted via SSH tunnel, only specific sender, static ARP tables against poisoning & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Script kiddie \& Skilled hacker: Hijack session, eavesdrop on communication & HTTPS only, timeout, clean termination  & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Insider, Script kiddie \& Skilled hacker: Denial of service, generate load & No access to intranet, contract with ISP, may implement backup network  & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Corporate espionage: Obtain backup & Backup done by system administrator only, stored off site in secured location (safe) & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Nature: Damage because of environmental impact (water, fire, etc.) & Stored off site, meaning other impacts as on site, stored in safe that withstands rough environmental influences & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}


\subsubsection*{{\it Evaluation assetKey archive encryption key}}
\begin{footnotesize}
\begin{prettytablex}{lXp{6.5cm}lll}
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Corporate espionage \& Skilled hacker: Obtain key & Key is stored non digitally off site in a bank vault & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Corporate espionage \& Insiders: obtaining key (theft) & Access to key limited to few people with no administrative access to the system, stored in bank vault & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Corporate espionage \& Skilled hacker: Obtain key & Access control, intrusion detection, system not accessible from network, monitoring and logging, update systems & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Bribery, corruption, threat of violence & contractual agreement to obey non-disclosure policies, contact police & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Careless handling of private key, workstation & Training, contractual agreement to obey usage policies & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Serious illness, accident, instant dismissal & Contractual requirement for precise documentation of the system, configurations, passwords etc. & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Bribery, corruption, threat of violence & contractual agreement to obey non-disclosure policies, contact police & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Unintended misconfiguration leading to service outage or disclosure of data & Backup, only certified and experienced employees, maintenance only during night time & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
\arabic{threadnr}\addtocounter{threadnr}{1} & Bribery, corruption, threat of violence & contractual agreement to obey non-disclosure policies, contact police & {\it } & {\it } & {\it } \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Careless handling of key & Contractual agreement to obey usage policies & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
...
No. & Threat & Implemented/planned countermeasure(s) & L & I & Risk \\
\hline
\arabic{threadnr}\addtocounter{threadnr}{1} & Theft or disclosure of confidential/sensitive data & Experienced and certified personnel, security measures, external reviews & {\it } & {\it } & {\it } \\
\hline
\end{prettytablex}
\end{footnotesize}
