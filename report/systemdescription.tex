...
The application which implements the certificate authority (CA) functionality is written in python and uses the opensource library \texttt{pyOpenSSL} to create, revoke and sign certificates and additionally the library \texttt{m2crypto} to verify if a given certificate is valid. The CA application communicates with the database via \texttt{SQLAlchemy}, a Python SQL toolkit and object relational mapper, which provides us a database interaction without SQL-Statements. 

DESCRIBE WHAT YOU IMPLEMENTED IN WHAT LANGUAGES, WHICH LIBRARIES WERE USED, HOW RPC IS IMPLEMENTED, WHAT WAS DONE IN TERMS OF SECURTY, DELETION OF PRIVATE KEY, HOW DOES THE PRIVATE KEY BACKUP WORK, MAYBE ALSO MENTION LOGGING.\newline
